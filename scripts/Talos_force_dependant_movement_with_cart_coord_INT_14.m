% Comments:
    ...
    % Talos moves in any direction (not just x or y or z axis) for
    % components of the applied force that are greater than lowest_F_value
    % (size of the move in that direction depends on the size of the
    % component of the force in that direction - which needs to be greater than
    % lowest_F_value)
...
while strcmp(swi1.Value,'On') % While the button is on
    ...
    % Set all the forces lower than lowest_F_value to 0
    
    forces_Talos = [F_x F_y F_z];

    for i=1:length(forces_Talos)
        if abs(forces_Talos(i)) < lowest_F_value
            forces_Talos(i)=0;
        end
    end
    ...
    % Positions for when all forces are lower than lowest_F_value
    ...
    if any(forces_Talos) % if any of the forces is not 0
        for i=1:num_limbs
